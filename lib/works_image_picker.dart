...
const String workskTypeImage = 'image';

/// Denotes that a video is being picked.
const String workskTypeVideo = 'video';
...
class WorksImagePicker {
  static const MethodChannel _channel =
  MethodChannel('works_image_picker');
  ...
  static Future<Map> pickImage(
    ...
    final Map info = await _channel.invokeMethod<Map>(
      'pickImage',
      <String, dynamic>{
        'source': source.index,
        'maxWidth': maxWidth,
  ...
  /// in this call. You can then call [retrieveLostData] when your app relaunches to retrieve the lost data.
  static Future<Map> pickVideo({
    @required ImageSource source,
    String subPath,
  }) async {
    assert(source != null);
    final Map videoInfo = await _channel.invokeMethod<Map>(
      'pickVideo',
      <String, dynamic>{
        'source': source.index,
        'subPath':subPath ?? ""
  ...
  static Future<LostDataResponse> retrieveLostData() async {
    ...
    assert(type == workskTypeImage || type == workskTypeVideo);

    RetrieveType retrieveType;
    if (type == workskTypeImage) {
      retrieveType = RetrieveType.image;
    } else if (type == workskTypeVideo) {
