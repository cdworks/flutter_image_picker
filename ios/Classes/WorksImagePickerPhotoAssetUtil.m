...
@implementation WorksImagePickerPhotoAssetUtil
...
+ (NSString *)saveImageWithOriginalImageData:(NSData *)originalImageData
  ...
  NSString *suffix = workskFLTImagePickerDefaultSuffix;
  WorksFLTImagePickerMIMEType type = workskFLTImagePickerMIMETypeDefault;
  NSDictionary *metaData = nil;
  // Getting the image type from the original image data if necessary.
  if (originalImageData) {
    type = [WorksImagePickerMetaDataUtil getImageMIMETypeFromImageData:originalImageData];
    suffix =
        [WorksImagePickerMetaDataUtil imageTypeSuffixFromType:type] ?: workskFLTImagePickerDefaultSuffix;
    metaData = [WorksImagePickerMetaDataUtil getMetaDataFromImageData:originalImageData];
  }
  if (type == WorksFLTImagePickerMIMETypeGIF) {
    GIFInfo *gifInfo = [WorksImagePickerImageUtil scaledGIFImage:originalImageData
                                                      maxWidth:maxWidth
                                                     maxHeight:maxHeight];
...
+ (NSString *)saveImageWithPickerInfo:(nullable NSDictionary *)info
  ...
  return [self saveImageWithMetaData:metaData
                               image:image
                              suffix:workskFLTImagePickerDefaultSuffix
                                type:workskFLTImagePickerMIMETypeDefault
                        imageQuality:imageQuality];
}
...
+ (NSString *)saveImageWithMetaData:(NSDictionary *)metaData
                              image:(UIImage *)image
                             suffix:(NSString *)suffix
                               type:(WorksFLTImagePickerMIMEType)type
                       imageQuality:(NSNumber *)imageQuality {
  NSData *data = [WorksImagePickerMetaDataUtil convertImage:image
                                                usingType:type
                                                  quality:imageQuality];
  if (metaData) {
    data = [WorksImagePickerMetaDataUtil updateMetaData:metaData toImage:data];
  }
