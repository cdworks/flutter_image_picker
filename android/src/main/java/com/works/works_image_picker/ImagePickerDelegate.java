...
public class ImagePickerDelegate
  ...
  @VisibleForTesting static final int REQUEST_CODE_CHOOSE_IMAGE_FROM_GALLERY = 0x2342;
  @VisibleForTesting static final int REQUEST_CODE_TAKE_IMAGE_WITH_CAMERA = 0x2343;
  @VisibleForTesting static final int REQUEST_EXTERNAL_IMAGE_STORAGE_PERMISSION = 0x2344;
  @VisibleForTesting static final int REQUEST_CAMERA_IMAGE_PERMISSION = 0x2345;
  @VisibleForTesting static final int REQUEST_CODE_CHOOSE_VIDEO_FROM_GALLERY = 0x2352;
  @VisibleForTesting static final int REQUEST_CODE_TAKE_VIDEO_WITH_CAMERA = 0x2353;
  @VisibleForTesting static final int REQUEST_EXTERNAL_VIDEO_STORAGE_PERMISSION = 0x2354;
  @VisibleForTesting static final int REQUEST_CAMERA_VIDEO_PERMISSION = 0x2355;
  ...
  void retrieveLostImage(MethodChannel.Result result) {
    ...
    if (path != null) {
      ...
      Bundle bundle = imageResizer.resizeImageIfNeeded(path, maxWidth, maxHeight, imageQuality);

      String finalImagePath = bundle.getString("path");
//      HashMap info = new HashMap();
//      info.put("path",finalImagePath);
//      info.put("width",bundle.getInt("width",0));
//      info.put("height",bundle.getInt("height",0));
//      finishWithSuccessWithInfo(info);

      resultMap.put(cache.MAP_KEY_PATH, finalImagePath);
      resultMap.put("width",bundle.getInt("width",0));
      resultMap.put("height",bundle.getInt("height",0));
  ...
  public void takeVideoWithCamera(MethodCall methodCall, MethodChannel.Result result) {
    ...
    launchTakeVideoWithCameraIntent((String) methodCall.argument("subPath"));
  }

  private void launchTakeVideoWithCameraIntent(String subPath) {
...
//    activity.startActivityForResult(intent, REQUEST_CODE_TAKE_VIDEO_WITH_CAMERA);
  ...
  public boolean onRequestPermissionsResult(
    ...
    switch (requestCode) {
      ...
      case REQUEST_CAMERA_VIDEO_PERMISSION:
        if (permissionGranted) {
          launchTakeVideoWithCameraIntent((String) this.methodCall.argument("subPath"));
        }
        break;
      default:
        return false;
  ...
  public boolean onActivityResult(int requestCode, int resultCode, Intent data) {
    switch (requestCode) {
      ...
      case REQUEST_CODE_TAKE_VIDEO_WITH_CAMERA:
        handleCaptureVideoResult(requestCode,resultCode,data);
        break;
      default:
        return false;
    }
  ...
  private void handleCaptureVideoResult(int requestCode, int resultCode, Intent data) {
    if (resultCode == Activity.RESULT_OK) {
      ...
      if (cursor.moveToFirst()) {
        ...
        duration = (int)(getDuration(filePath)/1000);
//                cursor
//                .getInt(cursor
//                        .getColumnIndexOrThrow(MediaStore.Video.Media.DURATION));
        Log.d("imagePick", "duration:"+duration);
      }
      if(cursor!=null)
      {
        cursor.close();
      ...
      handleVideoResult(filePath,duration);
...
//          });
  ...
  private void handleImageResult(String path, boolean shouldDeleteOriginalIfScaled) {
    if (methodCall != null) {
      ...
      if (bmp == null) {
        finishWithError("not decode bitmap", "file not found or not to decode bitmap.");
        return;
      }
      HashMap info = new HashMap();
      info.put("path",path);
      info.put("width",bmp.getWidth());
      info.put("height",bmp.getHeight());
      finishWithSuccessWithInfo(info);
    }
  }

  private void handleVideoResult(String path,long duration) {

    File file = new File(FileUtils.getImagePath((String) methodCall.argument("subPath")), "thvideo" + System.currentTimeMillis());
    try {
      FileOutputStream fos = new FileOutputStream(file);
      Bitmap ThumbBitmap = ThumbnailUtils.createVideoThumbnail(path, MediaStore.Images.Thumbnails.MINI_KIND);
      ...
      info.put("duration",(double)duration);
...
//  }
  ...
  private void finishWithSuccessWithInfo(HashMap info) {
    if(info != null)
    {
      if (pendingResult == null) {
        cache.saveResult((String)info.get("path"), null, null);
        return;
      }
